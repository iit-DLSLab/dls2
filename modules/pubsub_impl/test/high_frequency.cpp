...
int main(int /*argc*/, char ** /*argv*/)
{
	...
	std::vector<std::shared_ptr<dls::version2::Publisher<StringMsgPubSubType>>> publishers;
	std::vector<std::shared_ptr<SubscriberTestStruct>>                          subscribers;

	for(size_t i = 0; i != COUNT_OF_PUBLISHERS; ++i)
	{
		publishers.push_back(
			std::make_shared<dls::version2::Publisher<StringMsgPubSubType>>
			(topic)
		);
	}
	...
	for(size_t i = 0; i != COUNT_OF_SUBSCRIBERS; ++i)
	{
		...
		(
			std::make_shared<SubscriberTestStruct>()
		);
	}
