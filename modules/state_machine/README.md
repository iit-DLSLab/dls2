...
An activity is associated with each state. The activity describes the work performed when the state machine is in that state and can handle the transition to the next states based on the occurrence of the events.
...
It can be run autonomously. For example, you can run the _start()_ function in a thread. The state transitions can also be explicitly triggered, by calling _transit(event)_. 
