...
{
 std::lock_guard<std::mutex> lock(this->pose_mutex);
 return commons::quatToRPY(this->quaternion);
}
...
{
 std::lock_guard<std::mutex> lock(this->pose_mutex);
 return commons::quatToRotMat(this->quaternion);
}
...
Pose::transformation_matrix_t Pose::toTransformationMatrix() const
{
	...
	{
		std::lock_guard<std::mutex> lock(this->pose_mutex);
		T.block<3,3>(0,0) = commons::quatToRotMat(this->quaternion);//this->quaternion.toRotationMatrix();
