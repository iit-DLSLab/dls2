...
StanceStatus::StanceStatus(const std::shared_ptr<robotlib::RobotBase> robot)
	: frame_id_(""), sequence_id_(0), timestamp_(0.0), stance_status_(robot->makeLegDataMap<bool>(false))
{
}

StanceStatus::StanceStatus(StanceStatus &stance_status)
	: frame_id_(stance_status.frame_id_), sequence_id_(stance_status.sequence_id_), timestamp_(stance_status.timestamp_), stance_status_(stance_status.stance_status_)
{
}
...
void StanceStatus::setDataFromWrapperBase(WrapperBase *wrapper_base)
{
	// Assuming wrapper is of type Wrapper<MsgType>
	*this = static_cast<StanceStatus &>(*wrapper_base);
}