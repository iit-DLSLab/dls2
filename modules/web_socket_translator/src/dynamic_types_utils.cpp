        ...
        const TypeIntrospectionCollection& numeric_type_names)
{
    ...
    {
        type_names.push_back(std::get<dls::DatumLabel>(type_name));
    }
    return type_names;
}
        ...
        const std::string& separator /* = "/" */)
{
    ...
    std::cout << "Getting types for type member " << base_type_name << " of kind " + std::to_string(kind) << std::endl;
        ...
        case eprosima::fastrtps::types::TK_ARRAY:
        {
            ...
            for (MemberId member_id = 0; member_id < this_array_size; member_id++)
            {
                ...
                get_introspection_type_names(
                    base_type_name + std::string("[" + std::to_string(member_id) + "]"),
                    internal_type,
                    numeric_type_names,
                    string_type_names,
                    new_members_tree,
        ...
        TypeIntrospectionNumericStruct& numeric_data_result)
{
    ...
    {
        throw std::runtime_error("Vector of struct and result sizes mismatch in get_introspection_numeric_data");
    }
    ...
    for (long unsigned int i = 0; i < numeric_type_names.size(); ++i)
    {
        ...
        TypeIntrospectionStringStruct& string_data_result)
{
    ...
    {
        throw std::runtime_error("Vector of struct and result sizes mismatch in get_introspection_string_data");
    }

    // It is used by index of the vector to avoid unnecessary lookups.
    // Vectors must be sorted in the same order => creation order given in get_introspection_type_names
    for (long unsigned int i = 0; i < string_type_names.size(); ++i)
    {
        ...
        const TypeKind& kind)
{
    ...
    switch (kind)
    {
        ...
        default:
            ...
            throw std::runtime_error("Member wrongly set as numeric");
    }
}
        ...
        const TypeKind& kind)
{
    ...
    switch (kind)
    {
        ...
        default:
            // The rest of values should not arrive here (are cut before when creating type names)
            throw std::runtime_error("Member wrongly set as string");
    }
}
...
jsonPair createJsonSchema(const std::string& title, const DynamicType_ptr& type)
{
    std::stringstream ss;
    ...
    ss << "\"$comment\": \"Generated by dls\"," << std::endl;
    ss << "\"type\": \"object\"," << std::endl;
    ss << "\"properties\": {" << std::endl;

    getJsonContent(title, type, ss, jsonVar);
        ...
        nlohmann::json& jsonVar)
{
    ...
    switch (kind)
    {
        ...
        case eprosima::fastrtps::types::TK_FLOAT128:
            // Add numeric value
            jsonSchema << "\"" << base_type_name << "\": {" << std::endl;
            jsonSchema << "\"type\": \"number\"," << std::endl;
            jsonSchema << "\"description\": \"" << base_type_name << "\"" << std::endl;
            jsonSchema << "}";

            jsonVar[base_type_name] = 0.0;
            break;
        ...
        case eprosima::fastrtps::types::TK_ENUM:
            // Add string value
            jsonSchema << "\"" << base_type_name << "\": {" << std::endl;
            jsonSchema << "\"type\": \"string\"," << std::endl;
            jsonSchema << "\"description\": \"" << base_type_name << "\"" << std::endl;
            jsonSchema << "}";

            jsonVar[base_type_name] = "";
            break;
        ...
        case eprosima::fastrtps::types::TK_ARRAY:
        {
            ...
            jsonSchema << "\"" << base_type_name << "\": {" << std::endl;
            jsonSchema << "\"type\": \"array\"," << std::endl;
            jsonSchema << "\"items\": {" << std::endl;

            jsonVar[base_type_name] = nlohmann::json::array();

            if(internal_type->get_kind() <= 11)
            {
                jsonSchema << "\"type\": \"number\"" << std::endl;

                for (MemberId member_id = 0; member_id < this_array_size; member_id++)
                {
                    jsonVar[base_type_name][member_id] = 0.0;
                }
            }
            else
            {
                ...
                for (MemberId member_id = 0; member_id < this_array_size; member_id++)
                {
                    jsonVar[base_type_name][member_id] = "";
                }
            }
        ...
        case eprosima::fastrtps::types::TK_STRUCTURE:
        {
            ...
            for (const auto& members : members_by_name)
            {
                ...
                if (&members != &(*members_by_name.rbegin()))
                    jsonSchema << ",";
        ...
        unsigned int array_indexes /* = 0 */)
{
    ...
    switch (kind)
    {
        ...
        {
            // Add numeric value
            jsonVar[base_type_name] = get_numeric_type_from_data(data, array_indexes, kind);
            break;
        }
        ...
        case eprosima::fastrtps::types::TK_ENUM:
            // Add string value
            jsonVar[base_type_name] = get_string_type_from_data(data, array_indexes, kind);
            break;
        ...
        case eprosima::fastrtps::types::TK_ARRAY:
        {
            ...
            if(internal_type->get_kind() <= 11)
            {
                for (MemberId member_id = 0; member_id < this_array_size; member_id++)
                {
                    jsonVar[base_type_name][member_id] = get_numeric_type_from_data(data, member_id, internal_type->get_kind());
                }
            }
            else
            {
                for (MemberId member_id = 0; member_id < this_array_size; member_id++)
                {
                    jsonVar[base_type_name][member_id] = get_string_type_from_data(data, member_id, internal_type->get_kind());
                }
            }
            break;
        }
        ...
        case eprosima::fastrtps::types::TK_STRUCTURE:
        {
            ...
            for (const auto& members : members_by_name)
            {
                ...
                if(members.second->get_descriptor()->get_type()->get_kind() ==  eprosima::fastrtps::types::TK_ARRAY)
                {
                    child_data = data->loan_value(members.second->get_id());
                    data->return_loaned_value(child_data);
                }
                else
                {
