...
Ther *create_t*, *destroy_t* and *run* functions has to be implemented implemented in any class inheriting from PeriodicPluginBase. See [How to write a periodic plugin](#how-to-write-a-periodic-plugin) section.

When creating an input, the plugin performs two steps:
* create a data reader, subscribed to the desired topic
* store the pointer to the input variable, used by *read()*. This function automatically gets the last message read by each data reader and updates each corresponding input variable trough its pointer
...
In this section it is described which are the components that a specific plugin must have and how to implement them. The [Overview](#overview) section describes the components. The [Steps](#steps) section instead, tells you directly how to create a plugin, which it basically follows what it is written in the overview. So you could directly go to the [Steps](#steps), despite it is kindly suggested to read the overview.
...
* create a sets of data readers, for getting the module inputs from the DLS2 network
* create a sets of data writers, for sending the module outputs to the DLS2 network
...
* *module.run(inputs, outputs)*: it calls the run function fo the module, which is responsible for implementing the module logic. Here *module* is the instace of your module class
* *write()*: it seds all the outputs of the module to the DLS2 network
...
* [create a repo on GitLab](https://gitlab.advr.iit.it/projects/new)
...
* call the following command
        ...
        create_periodic_plugin module_name module_type

    where \<module_name> is the name of your module and \<module_type> can be:
    * hardwares
    * controllers
    * state_estimators
    * motion_generations
    ...
    * plugin/console_commands: where you develop your console command class
    * plugin/messages: where you develop custom messages
    * plugin/topics: where you develop custom topics
        
As you can see, in the project structure there is also the possibility to define custom messages and topics, that you can use in your plugin. The procedure to create a message is described [here-TODO](TODO).

In [examples/periodic/skeleton](examples/periodic//skeleton/) you can see how this project structure looks like.

The skeleton is providing you also a guide on how to write .hpp and .cpp files of the module and plugin. In these files, there are several placeholders identified by comments between /**/. Each placeholder, must be substituted by what the placeholder description says. For example, if I have created a module class called StanceDetection, I could substitute the following placeholders

    private:
        /*module_class_name*/ /*variable_name*/_;

with

    private:
        StanceDetection stance_detection_;

The same happens for other placeholders.

In [/examples/periodic/dummy_plugin](examples/periodic/dummy_plugin/) there is a full example of how the project should look like.
