    ...
    , data_type_configuration_(data_type_configuration)
{
    ...
    static_type_ = is_type_static(type);
        ...
        eprosima::fastdds::dds::DataReader* reader)
{
    ...
    while (!stop_ && read_ret == eprosima::fastrtps::types::ReturnCode_t::RETCODE_OK)
    {
                ...
                info.instance_state == eprosima::fastdds::dds::InstanceStateKind::ALIVE_INSTANCE_STATE)
        {
            ...
            // Get timestamp
            double timestamp = get_timestamp_seconds_numeric_value(info.reception_timestamp);

            // Get data in already created structures
            get_introspection_numeric_data(
                numeric_data_info_,
                data_,
                numeric_data_);

            get_introspection_string_data(
                string_data_info_,
                data_,
                string_data_);
...
{
    return get_introspection_type_names(numeric_data_info_);
}

std::vector<std::string> ReaderHandler::string_data_series_names() const
{
    return get_introspection_type_names(string_data_info_);
}
