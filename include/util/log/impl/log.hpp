...
namespace logging
{
	...
	namespace impl
	{
		...
		///
		/// Empty class that wraps StreamManipulator. Can be passed into a log
		/// stream to perform manipulations
		template <StreamManipulator manip>
		class StreamManip{};

		/// Log Helper class
		///
		/// This class is instantiated when a write is made to a LogStream. It
		/// keeps track of all the writes that are made to the log stream and
		/// sends it in one go to the main log stream class when it encounters
		/// logging::StreamManip<ENDL>. That way, an entire log stream is
		/// written such that concurrent writes won't interleave.
		template <LogLevel loglevel>
		class LogInput
		{
		...
		public:
			/// Start logging
			///
			/// @return a LogInput object which takes care of building the log
			/// and sending it back as a single string object. This is done to
