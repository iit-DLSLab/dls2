...
bool ControlLayer::activateController(const Controller::ID_t &ID)
{
	...
	decltype(this->controllers_b.find(ID)) pair_it;
	{
		...
		if((controller_pid = fork()) == 0)
		{
			...
			(
				#ifndef NDEBUG
					"/home/hendrik/dev/dls2_deploy/dls2/bin//child_process_launcher",
				#else
					"/usr/bin/dls2/child_process_launcher",
				#endif
				ID.c_str(),
				"controller",
				"hyq",
			...
			// execl("/usr/bin/dls2/dls_controller_process", ID.c_str(), ID.c_str(), (char *)NULL);
		...
		pData->pSubscriber =
			...
			(
				...
				std::string(topics::control_signal_base) + ID
			);
...
bool ControlLayer::activateGaitGenerator(const GaitGenerator::ID_t &ID)
{
	...
	if(this->pGait_generator_data->gait_generator_pid == 0)
	{
		...
		// execl("/usr/bin/dls2/dls_gait_generator_process", ID.c_str(), ID.c_str(), (char *)NULL);
